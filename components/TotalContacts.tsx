...
const TotalContacts: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setModalMessage('Contact created successfully!');
        setModalType('success');
        setShowModal(true);
      } else {
        console.log(newContact)
        setModalMessage('Failed to create contact!');
        setModalType('error');
        setShowModal(true);
      }
    } catch (error) {
  ...
  const handleUpdateContact = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setModalMessage('Failed to update contact!');
        setModalType('error');
        setShowModal(true);
      }
    } catch (error) {
  ...
  const handleDelete = () => {
    setIsMultiple(true);
    setModalMessage('Are you sure you want to delete the selected contact/s?');
...
const confirmMultipleDelete = async () => {
  try {
    ...
    if (allSuccess) {
      fetchContacts();
      setSelectedRows([]);
      setModalMessage('Contact/s deleted successfully!');
      setModalType('success');
      setShowModal(true);
    } else {
      setModalMessage('Failed to delete contact/s!');
      setModalType('error');
      setShowModal(true);
    }
  } catch (error) {
...
.filter((contact) => {
  ...
  const matchesCategory = filterCategory
    ? contact.category === categories.find(category => category.name === filterCategory)?.id
    : true;
  ...
  const handleRowDeletion = (id: number) => {
    setSelectedRow(id);
    setModalMessage('Are you sure you want to delete the selected contact?');
    setModalType('confirmation');
    setShowModal(true);
  }
  ...
  const handleDeleteRow = async () => {
    if(selectedRow !== null){
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/contacts/${selectedRow}`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        });
  ...
  return (
    <div>
      ...
      <div className="flex space-x-2">
        <div>
        <button
          className="bg-blue-500 w-28 text-white px-2 py-1 rounded flex items-center"
          onClick={() => setDropdownVisible(!dropdownVisible)}
        >
          <FaFilter className="mr-2" />
          {filterCategory ? filterCategory : 'Filters'}
        </button>
          {dropdownVisible && (
    ...
    {/* Table */}
    <table className="table-auto text-sm w-full bg-white shadow rounded font-titillium">
            ...
            <tr key={contact.id}>
              ...
              <td className="px-4 py-2 flex space-x-2">
                ...
                <button
                  ...
                  onClick={() => handleRowDeletion(contact.id)}
                >
                  <FaTrash />
                </button>
              </td>
    ...
    {showModal && (
      <NotificationModal
        ...
        onConfirm={modalType === 'confirmation' ? (isMultiple ? confirmMultipleDelete : handleDeleteRow) : undefined}
      />
    )}
